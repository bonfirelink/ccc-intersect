  ...
  Vue.filter('formatTime', function (value, format, timezones) {
    ...
    if (timezones) {
      ...
      if (timzoneDate && date !== "00:00") {
        date = date + seperator + `<span class="opacity-75">` + timzoneDate.tz('America/Costa_Rica').format(format) + '</span>' +
          seperator +  `<span class="opacity-50">` + timzoneDate.tz('Asia/Kolkata').format(format) + '</span>'
      }
    }
